...
class turtlebot_move():
	...
	rospy.logwarn("x: " + str(x) + " self.x :" + str(self.x) + "y: " + str(x) + " self.y :" + str(self.x))
	rospy.logwarn("diff_x: " + str(diff_x) + " diff_y :" + str(diff_y) + " theta: " + str(theta))
	    ...
	    rospy.logwarn("### angular: = " + str(angular))
            if abs(angular) > 0.2:
...
if __name__ == '__main__':
	...
	goalY = 0
 	j = 1
	stage_pub = rospy.Publisher('/stage', Int32, queue_size=10)
	stage = Int32()
	rate = rospy.Rate(10)
	rate.sleep()
	for i in range(1,5):
